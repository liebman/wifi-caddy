...
/// (storage params, notify channel, update callback).
///
/// Implementations are generated by the `WifiCaddyConfig` derive macro
/// when `#[config_server]` is present on the struct.
#[doc(hidden)]
#[allow(async_fn_in_trait)]
pub trait ConfigServer: ConfigType {
    /// Type returned by [`init_update_channel`](Self::init_update_channel).
    /// `ConfigUpdateReceiver` when `#[config_notify]` is present, `()` otherwise.
    type UpdateReceiver;
    ...
    /// Returns `Some(...)` when `#[config_notify]` is present, `None` otherwise.
    fn on_updated() -> Option<&'static (dyn Fn(<Self as ConfigApi>::ChangedSet) + Send)>;

    /// Initialize the update channel and return a receiver handle.
