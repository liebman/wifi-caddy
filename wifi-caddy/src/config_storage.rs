...
//! # Value size limit
//!
//! [`MAX_VALUE_SIZE`] bounds the scratch buffers the storage path uses, and with
//! them the largest single config value (after serialization) that can round-trip:
//! a longer value fails to store with [`ConfigError::BufferTooSmall`] — which
//! reports the size it needed — and, once such a value is in flash, fails to load
//! with [`ConfigError::Backend`], which surfaces as a config load error at boot.
//!
//! It costs stack, not static RAM: `get_value`/`set_value`, and the flash
//! backend's fetch/store buffers (which are sized from the same constant),
//! allocate one buffer per call inside whatever task stores the config. The config
//! fields this crate ships with fit easily (SSIDs are at most 32 bytes,
//! passphrases 63), so the default is kept small and applications with longer
//! `String` fields — a bearer token, a URL, an entity id — raise it with the
//! `WIFI_CADDY_MAX_VALUE_SIZE` build-time env var instead of every application
//! paying for the headroom.
...
// `MAX_VALUE_SIZE` — the longest single config value the storage path can
// round-trip — is generated by `build.rs` from the `WIFI_CADDY_MAX_VALUE_SIZE`
// env var (default 128). Its documentation lives in the generated file; see the
// "Value size limit" section of this module's docs for what it bounds.
include!(concat!(env!("OUT_DIR"), "/config_limits.rs"));
...
mod tests {
    ...
    /// A value exactly at the limit round-trips: the limit is inclusive.
    #[test]
    fn string_at_the_limit_round_trips() {
        let value = "a".repeat(MAX_VALUE_SIZE);
        let mut buf = [0u8; MAX_VALUE_SIZE];
        let len = value.to_bytes(&mut buf).unwrap();
        assert_eq!(len, MAX_VALUE_SIZE);
        assert_eq!(String::from_bytes(&buf[..len]).unwrap(), value);
    }

    /// One byte more than the limit is refused, and the error carries the size the
    /// value needed — that is the number an application sees as
    /// `buffer too small: need N bytes` when one of its config fields outgrows the
    /// limit (`MAX_VALUE_SIZE` is settable through `WIFI_CADDY_MAX_VALUE_SIZE`).
    #[test]
    fn string_over_the_limit_reports_the_size_it_needed() {
        let value = "a".repeat(MAX_VALUE_SIZE + 1);
        let mut buf = [0u8; MAX_VALUE_SIZE];
        assert_eq!(
            value.to_bytes(&mut buf),
            Err(ConfigError::BufferTooSmall(MAX_VALUE_SIZE + 1))
        );
    }
}
