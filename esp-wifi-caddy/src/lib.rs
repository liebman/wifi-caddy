...
// Socket counts per network stack, generated by `build.rs` (see the
// `ESP_WIFI_CADDY_AP_SOCKETS` / `ESP_WIFI_CADDY_STA_SOCKETS` env vars):
// `StackResources<N>` is allocated once per stack, and on ESP32 a socket slot
// costs 352 bytes plus a fixed per-stack overhead, so the counts are sized to
// what the portal and the application actually need rather than to a fixed 10.
include!(concat!(env!("OUT_DIR"), "/stack_sockets.rs"));
